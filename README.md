...
| `-NoBackup` | `switch` | off | Skip copying the original file to `<file>.bak` before writing (on by default; overwrites any existing backup). Backups are already skipped under `-WhatIf`. |
