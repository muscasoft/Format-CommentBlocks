function Get-WrappedLines {
    # Shared helper: greedy word-wrap of a list of words into lines of at most $MaxLength, each line
    # starts with $Prefix (indent + optional "# ").
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseSingularNouns', '', Justification = 'Get-WrappedLines returns a collection of lines by design; the plural is accurate, and this is an internal helper with no external callers.')]
    param(
        [string[]]$Words,
        [string]$Prefix,
        [int]$MaxLength
    ...
    foreach ($word in $Words) {
        ...
        elseif ($word.EndsWith('-----') -or ($current.Length + 1 + $word.Length) -le $MaxLength) {
            # "-----" (five dashes) marks a block of code and must stay on the current line even if
            # that means exceeding $MaxLength. This is unrelated to "---" (three dashes), which is
            # just the paragraph-end marker used in Format-CommentBlocks.
            $current += " $word"
        }
        else {
            $wrapped.Add($current)
...
function Get-LineDiff {
    # Computes a line-by-line diff between $Original and $New using Myers' O(ND) diff algorithm -
    # the same technique classic 'diff' tools use. Cost scales with the number of differences (D)
    # between the inputs, not with $Original.Count * $New.Count, so it stays fast even on large
    # files as long as the two inputs are mostly the same - which holds here, since reflowing
    # comments only changes a handful of lines out of the whole file.
    ...
    # v[offset + k] holds the furthest x reached on diagonal k so far. Diagonals run from -maxD to
    # +maxD, so every index is shifted by $offset to fit a 0-based array. The +2 (rather than +1)
    # leaves one extra slot for the d=0 boundary read, notably when both inputs are empty.
    $offset = $maxD
    ...
    # Walk the trace backwards from (n, m) to (0, 0) to recover the actual edit script, then reverse
...
function Show-ReflowDiff {
    # Groups the diff into "hunks": clusters of consecutive changes, with $Context unchanged lines
    # before and after for orientation (like git diff).
    param(
        [string]$FileName,
        [string[]]$Original,
        [string[]]$New,
    ...
    # Form clusters: changes that are less than (2 * Context) lines apart are merged into a single
    # hunk.
...
.PARAMETER NoBackup
    Skips backing up the original file before writing changes. By default,
    the original is copied to "<file>.bak" (overwriting any existing
    backup) before every write; pass -NoBackup to skip that. Backups are
    already skipped under -WhatIf, since nothing is written in that case
    either.
.PARAMETER Help
...
.EXAMPLE
    Format-CommentBlocks -Path .\MyScript.ps1 -NoBackup
#>
function Format-CommentBlocks {
    [CmdletBinding(SupportsShouldProcess, DefaultParameterSetName = 'Run')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseSingularNouns', '', Justification = 'Format-CommentBlocks is the published, already-released public function name; renaming now would be a breaking change.')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSReviewUnusedParameter', 'Help', Justification = 'Presence of -Help alone selects the Help parameter set; the switch value itself does not need to be read.')]
    param(
    ...
    process {
        ...
        foreach ($file in $Path) {
            ...
            # Matches "- ", "* ", "+ ", "1. " or "1) " at the start of a comment's text. Lines like
            # this are list items: they are always wrapped on their own, never merged with a
            # preceding or following line, regardless of trailing punctuation.
            ...
            while ($i -lt $lines.Count) {
                ...
                if ($lines[$i] -match $linePattern -and $Matches.text.Trim().Length -gt 0) {
                    ...
                    while ($true) {
                        ...
                        # Paragraph ends on a period, on three dashes, or when the line itself is a
                        # list item (list items always stand alone).
